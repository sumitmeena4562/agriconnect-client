  ...
  return (
    <div className="w-80 border-l border-slate-150 flex flex-col bg-white h-full overflow-hidden shrink-0">
          ...
          {Object.keys(vehicleTemplates).map(key => {
            ...
            return (
              <button
                ...
                className={`py-1.5 px-2 rounded-xl text-[9.5px] font-extrabold flex items-center gap-1.5 transition-all border text-left cursor-pointer ${
                  isSelected 
      ...
      <div className="p-4 border-b border-slate-150 shrink-0 bg-slate-50/50 flex items-center justify-between">
        <h4 className="text-[12px] font-black text-slate-800 uppercase tracking-wider">Load Sequence</h4>
        <button
          onClick={() => {
        ...
        {deliveryStops.map((stop, idx, arr) => {
          ...
          return (
            <div 
              ...
              className="flex items-center justify-between p-2.5 bg-white hover:bg-slate-50 rounded-xl transition-all border border-slate-200/80 cursor-grab active:cursor-grabbing shadow-xs"
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[9px] text-slate-450 font-semibold pl-7 truncate leading-none mt-1">
                  🏪 {order?.vendor?.name}
                </p>
                <div className="flex items-center gap-2.5 pl-7 mt-2">
                  <span className="text-[8px] font-black bg-slate-100 border border-slate-200 text-slate-500 px-1.5 py-0.5 rounded uppercase">
